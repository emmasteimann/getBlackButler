...
class GetBlackButler
  ...
  def prep_chapters
    ...
    page = @agent.get("http://www.mangareader.net/102/kuroshitsuji.html")
    @chapter_markers = {}
    i = 1
    @first_page = 0
    load_chapter = 1
  ...
  def chapter_directory(chapter_id)
    "blackbutler_comics/blackbutler_chapter_#{chapter_id}"
  end
  ...
  def zip_file_path(chapter_id)
    ...
    "#{directory_name}/blackbutler_chapter_#{chapter_id}.cbz"
  end
...
GetBlackButler.new_download(nil)
